...
const C = {
  topbar: "#1C0A0A", sidebar: "#FFFFFF", bg: "#F4F3F1", card: "#FFFFFF",
...
const SIT_COLOR = { 10: C.blue, 20: C.amber, 30: C.green, 40: C.text3 };
const SIT_LABEL = { 10: "Ativa", 20: "Pausada", 30: "Concluída", 40: "Cancelada" };
const SIT_BG    = { 10: C.blueBg, 20: C.amberBg, 30: C.greenBg, 40: "#F5F5F5" };
...
const Kpi = ({label,value,sub,color=C.red,icon}) => (
  <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:10,
...
export default function Produtividade() {
  ...
  const optsProj = useMemo(()=>[...new Set(rawHoras.map(h=>h.task_type).filter(Boolean))].sort(),[rawHoras]);
  ...
  const horas = useMemo(()=>rawHoras.filter(h=>{
    ...
    if(projSel.length   && !projSel.includes(h.task_type))    return false;
    return true;
  ...
  const tarefas = useMemo(()=>rawTarefas.filter(t=>{
    ...
    return true;
  }),[rawTarefas,executores,workspaces]);
  ...
  // ── Horas por situação de tarefa (novo!) ───────────────────────────────────
  ...
  const horasPorSituacao = useMemo(()=>{
    const acc={};
    horas.forEach(h=>{
      const t = tarefasMap[h.task_id];
      const sit = t?.situation;
      let label;
      if(!h.task_id)      label="Avulso";
      else if(!t)         label="Sem tarefa";
      else                label=SIT_LABEL[sit]||"Outro";
      acc[label]=(acc[label]||0)+(h.minutes||0);
    });
    return Object.entries(acc)
      .map(([name,min])=>({name,horas:+(min/60).toFixed(1)}))
      .sort((a,b)=>b.horas-a.horas);
  },[horas,tarefasMap]);

  const horasConcluidas = useMemo(()=>
    horas.filter(h=>tarefasMap[h.task_id]?.situation===30).reduce((s,h)=>s+(h.minutes||0),0)
  ,[horas,tarefasMap]);

  const horasAbertas = useMemo(()=>
    horas.filter(h=>tarefasMap[h.task_id]?.situation===10).reduce((s,h)=>s+(h.minutes||0),0)
  ,[horas,tarefasMap]);
  ...
  const custoCliente = useMemo(()=>
    horasPorWs.map(w=>({...w,custo:w.horas*taxaHora,
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Inter','Segoe UI',system-ui,sans-serif",color:C.text}}>
      ...
      <div style={{background:C.card,borderBottom:`1px solid ${C.border}`,
        ...
        <MultiSelect label="Tipo tarefa" options={optsProj} value={projSel}    onChange={setProjSel}/>
        <div style={{width:1,height:22,background:C.border2}}/>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
        ...
        <button onClick={()=>setShowCusto(v=>!v)} style={{
          background:showCusto?C.redLight:"transparent",color:showCusto?C.red:C.text3,
      ...
      <div style={{padding:"24px 28px",maxWidth:1600,margin:"0 auto"}}>
        ...
        {!loading&&!err&&(<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:12,marginBottom:24}}>
            ...
            <Kpi label="Membros Ativos"  value={horasPorExec.length} sub="com horas no período"             color={C.cyan}   icon="◉"/>
          </div>

          {/* ── HORAS CONCLUÍDAS VS ABERTAS (novo!) ── */}
          <div style={{marginBottom:20}}>
            <Card>
              <SectionTitle>Horas por Situação da Tarefa</SectionTitle>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:16,marginBottom:20}}>
                {[
                  {label:"Em tarefas concluídas", min:horasConcluidas, color:C.green, bg:C.greenBg},
                  {label:"Em tarefas ativas",     min:horasAbertas,    color:C.blue,  bg:C.blueBg},
                  {label:"Total apontado",        min:totalMin,        color:C.red,   bg:C.redLight},
                  {label:"Eficiência (concluído)", value:totalMin>0?fmtPct((horasConcluidas/totalMin)*100):"—", color:C.amber, bg:C.amberBg},
                ].map((item,i)=>(
                  <div key={i} style={{background:item.bg,border:`1px solid ${item.color}22`,borderRadius:10,padding:"14px 18px"}}>
                    <p style={{color:item.color,fontSize:10,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",margin:"0 0 8px"}}>{item.label}</p>
                    <p style={{color:C.text,fontSize:24,fontWeight:900,margin:0}}>
                      {item.value || fmtH(item.min)}
                    </p>
                  </div>
                ))}
              </div>
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={horasPorSituacao} barSize={36}>
                  ...
                  <Bar dataKey="horas" name="Horas" radius={[6,6,0,0]}>
                    {horasPorSituacao.map((entry,i)=>{
                      const colors={"Concluída":C.green,"Ativa":C.blue,"Pausada":C.amber,"Cancelada":C.text3,"Avulso":C.purple,"Sem tarefa":C.cyan};
                      return <Cell key={i} fill={colors[entry.name]||PALETTE[i%PALETTE.length]}/>;
                    })}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
                  ...
                  <div key={i} style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                        <span style={{color:C.text,fontSize:12,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{m.name}</span>
          ...
          <p style={{textAlign:"center",color:C.text3,fontSize:11,marginTop:24}}>
            V4 Company ACM · {dateFrom} até {dateTo} · Ekyte → Supabase
          </p>
        </>)}
      </div>
    </div>
