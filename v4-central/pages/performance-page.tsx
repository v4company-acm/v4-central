...
function KpiCard({ label, value, sub, delta: d, invert = false, alert }: {
  label: string; value: string; sub?: string; delta?: number | null; invert?: boolean; alert?: string
}) {
  return (
    <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '16px 20px', minWidth: 0 }}>
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      <div style={{ background: '#f3f4f6', borderRadius: 6, height: 10, overflow: 'hidden' }}>
        <div style={{ width: `${Math.min(pctOfTop, 100)}%`, height: '100%', background: color, borderRadius: 6 }} />
      </div>
...
export default function PerformancePage() {
  ...
  const th: React.CSSProperties = { padding: '10px 12px', fontSize: 11, fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.06em', textAlign: 'left', borderBottom: '1px solid #f3f4f6' }
  const td: React.CSSProperties = { padding: '10px 12px', fontSize: 13, borderBottom: '1px solid #f9fafb', color: '#111' }
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#f9fafb', fontFamily: "'DM Sans', sans-serif" }}>
        ...
        {/* Header */}
        <div style={{ background: '#111', color: '#fff', padding: '16px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontWeight: 800, fontSize: 16 }}>
            <div style={{ background: '#D32F2F', borderRadius: 6, width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800 }}>V4</div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 24px' }}>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', marginBottom: 24 }}>
            ...
            <input type="date" style={{ padding: '6px 12px', borderRadius: 8, border: '1.5px solid #e5e7eb', fontSize: 13, background: '#fff' }} value={dateFrom} onChange={e => { setDateFrom(e.target.value); setPreset(0) }} />
            <span style={{ color: '#9ca3af', fontSize: 13 }}>até</span>
            <input type="date" style={{ padding: '6px 12px', borderRadius: 8, border: '1.5px solid #e5e7eb', fontSize: 13, background: '#fff' }} value={dateTo} onChange={e => { setDateTo(e.target.value); setPreset(0) }} />
          ...
          {error && (
            <div style={{ background: '#fee2e2', border: '1px solid #fca5a5', borderRadius: 10, padding: '12px 16px', color: '#991b1b', marginBottom: 20, fontSize: 13 }}>
              ⚠ {error}
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12, marginBottom: 24 }}>
            ...
            <KpiCard label="Conversões" value={fmt(conv)} delta={delta(conv, cConv)} sub={compare && cmp.length ? `Ant: ${fmt(cConv)}` : undefined} />
          ...
          <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '20px 24px' }}>
            <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '1px solid #e5e7eb' }}>
              <button style={tabStyle(tab === 'funil')} onClick={() => setTab('funil')}>🎯 Funil & Gaps</button>
            ...
            {tab === 'funil' && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
                <div>
                  ...
                  <FunnelBar label="Conversões" value={fmt(conv)} pctOfTop={imp ? (conv / imp) * 100 : 0} color="#b91c1c" />
                </div>
                <div>
            ...
            {tab === 'debug' && (
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                  ...
                  <tbody>
                    {current.length === 0 && (
                      <tr><td colSpan={11} style={{ ...td, textAlign: 'center', color: '#9ca3af', padding: 32 }}>Nenhum dado no período.</td></tr>
                    )}
                    {current.map((r, i) => (
                      <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                        ...
                        <td style={{ ...td, color: r.ctr < 0.02 ? '#dc2626' : '#16a34a', fontWeight: 600 }}>{pct(r.ctr)}</td>
                        <td style={td}>{fmt(r.conversions, 1)}</td>
                        <td style={{ ...td, color: r.conversion_rate < 0.05 ? '#dc2626' : '#16a34a', fontWeight: 600 }}>{pct(r.conversion_rate)}</td>
                        <td style={td}>{brl(r.cost)}</td>
                        <td style={td}>{r.conversions > 0 ? brl(r.cost / r.conversions) : '—'}</td>
                        <td style={{ ...td, color: r.search_impression_share < 0.5 ? '#dc2626' : '#16a34a', fontWeight: 600 }}>{pct(r.search_impression_share)}</td>
                        <td style={{ ...td, color: r.search_lost_impression_share_rank > 0.2 ? '#dc2626' : '#374151' }}>{pct(r.search_lost_impression_share_rank)}</td>
                        <td style={{ ...td, color: r.search_lost_impression_share_budget > 0.1 ? '#dc2626' : '#374151' }}>{pct(r.search_lost_impression_share_budget)}</td>
                      </tr>
                    ))}
                  </tbody>
                  {current.length > 0 && (
                    <tfoot>
                      <tr style={{ background: '#f3f4f6', fontWeight: 700 }}>
                        ...
                        <td style={td}>{pct(ctrV)}</td>
                        <td style={td}>{fmt(conv, 1)}</td>
                        <td style={td}>{pct(cvr)}</td>
                        <td style={td}>{brl(cost)}</td>
                        <td style={td}>{conv > 0 ? brl(cpa) : '—'}</td>
                        <td style={td}>{pct(impShare)}</td>
                        <td style={td}>{pct(lostRank)}</td>
                        <td style={td}>{pct(lostBudg)}</td>
                      </tr>
                    </tfoot>
                  )}
                </table>
